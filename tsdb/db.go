...
func BeyondSizeRetention(db *DB, blocks []*Block) (deletable map[ulid.ULID]struct{}) {
	...
	if db.opts.MaxPercentage > 0 {
		...
		if diskSize <= 0 {
			level.Warn(db.logger).Log("msg", "Unable to retrieve filesystem size of database directory (%s), skip percentage limitation and default to fixed size limitation", db.dir)
		} else {
			// apply percentage
